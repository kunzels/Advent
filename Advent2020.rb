...
LLLLLLLLLLLLLLLLL.LLLL.LLLLL.LLL.L.LLLLLL.LLLLLLLL...LLLLLLLLLLLLLLL.LLLLL.LLL..LLLL.LLLLLL".split("\n")
...
def seating(data)
    data = surround(data)
    former = data.clone.map(&:clone)
    current = data.clone.map(&:clone)
    doit = true
    while former != current || doit == true
        former = current.clone.map(&:clone)
        doit = false
  
        former.each_with_index do |row, idx|
            row.each_char.with_index do |seat, idx2|
                if seat == "*" || seat == "."
                    current[idx][idx2] = seat
                    next
                elsif seat == "L" || seat == "#"
                    square = [former[idx - 1][idx2 - 1], former[idx - 1][idx2], former[idx -1][idx2 + 1], former[idx][idx2 - 1],former[idx][idx2 + 1], former[idx + 1][idx2 - 1], former[idx + 1][idx2], former[idx + 1][idx2 + 1]]
                    newSeat = chunkCheck(square, seat)
                    current[idx][idx2] = newSeat
                end
            end
        end
    end
